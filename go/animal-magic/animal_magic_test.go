//nolint:gosec // In the context of this exercise, it is fine to use math.Rand
...
func TestWandEnergy(t *testing.T) {
	...
	if high != 0.0 {
		t.Errorf("GenerateWandEnergy() results are not uniformly distributed. " +
							"%.2f to %.2f should contain values.", low, high)
	}
}

func TestShuffleAnimals(t *testing.T) {
	const tests = 100
	animals := []string{"ant", "beaver", "cat", "dog", "elephant", "fox",
	"giraffe", "hedgehog"}
	foundDifferent := false
	var last []string
	var got []string
