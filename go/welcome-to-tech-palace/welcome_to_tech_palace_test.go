	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := WelcomeMessage(tt.customer); got != tt.want {
				t.Errorf("WelcomeMessage(\"%s\") = \"%s\", want \"%s\"",
									tt.customer, got, tt.want)
			}
		})
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := AddBorder(tt.welcomeMessage, tt.numStarsPerLine); got != tt.want {
				t.Errorf("AddBorder(\"%s\", %d) = \"%s\", want \"%s\"",
									tt.welcomeMessage, tt.numStarsPerLine, got, tt.want)
			}
		})
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := CleanupMessage(tt.oldMessage); got != tt.want {
				t.Errorf("CleanupMessage(\"%s\") = \"%s\", want \"%s\"",
									tt.oldMessage, got, tt.want)
			}
		})
	}
}
