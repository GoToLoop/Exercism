...
func TestNewCar(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got != tt.expected {
				t.Errorf("NewCar(%+v,%+v) = %+v; expected %+v",
									tt.car.speed, tt.car.batteryDrain, got, tt.expected)
			}
		})
	}
...
func TestNewTrack(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got != tt.expected {
				t.Errorf("NewTrack(%+v) = %+v; expected %+v",
									tt.track.distance, tt.track, tt.expected)
			}
		})
	}
}
...
func TestCanFinish(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Car can finish the track distance w/ initial battery < 100%",
			car: Car{
				speed:        2,
				batteryDrain: 3,
				battery:      25,
		...
		{
			name: "Car cannot finish the track distance w/ initial battery < 100%",
			car: Car{
				speed:        2,
				batteryDrain: 3,
				battery:      25,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got != tt.expected {
				t.Errorf("CanFinish(%#v, %#v) = %+v; expected %+v",
									tt.car, tt.track, got, tt.expected)
			}
		})
	}
}
