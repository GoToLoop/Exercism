...
func TestFixBirdCountDoesNotCreateNewSlice(t *testing.T) {
	...
	if reflect.ValueOf(got).Pointer() != reflect.ValueOf(counts).Pointer() {
		t.Error("it looks like that you are creating a new slice in the function " +
			"FixBirdCountLog - please make sure you are modifying the slice passed as argument")
	}
}
