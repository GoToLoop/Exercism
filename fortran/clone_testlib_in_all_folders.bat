...
:: The corresponding full path for "testlib":
SET FULL=%CD%\%FOLDER%

:: Loop over all subfolders to find all sub-subfolders named "testlib":
FOR /D %%d IN (*) DO (
    ...
    REM Check if current subfolder contains a "testlib" inside it:
    IF EXIST %SUBFOLDER% (
...
:CreateJunction
    :: Create a junction in the given location targeting "testlib":
    MKLINK /J %1 %FULL%
    EXIT /B
