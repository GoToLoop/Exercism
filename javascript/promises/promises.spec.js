...
/**
 * @template T
 * @typedef {import("./promises").Handler<T>} Handler
 */

describe('promises', () => {
  const failedCallback = Error('Failed callback');

  const createCallbackFn = (speed=0) =>
    /**
     * @template T
     * @param {T} value 
     * @param {Handler<T>} callback 
     */
    (value, callback) => setTimeout(() => callback(null, value), speed);

  const createFailedCallback = (speed=0) =>
    /** @param {Handler<Error?>} callback */
    (_, callback) => setTimeout(() => callback(failedCallback), speed);
  ...
  describe('race', () => {
    ...
    test('resolves with value of the fastest promise even if other slower' +
          ' promises fail', () => {
      const FAST = 'FAST';
      return expect(
    ...
    test('rejects if the fastest promise fails even if other slower' +
          ' promises succeed', () => {
      return expect(
  ...
  describe('any', () => {
    ...
    test('resolves when given no promises', () => {
      return expect(any([])).resolves.toEqual([]);
    });

    test('resolves when given no arguments', () => {
      return expect(any()).resolves.toBeUndefined();
    });
    ...
    test('resolves with value of the fastest successful promise even if' +
          ' slower promises fail', () => {
      const FAST = 'FAST';
      return expect(
    ...
    test('resolves with value of fastest successful promise even if faster' +
          ' promises fail', () => {
